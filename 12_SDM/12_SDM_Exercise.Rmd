...
```{r, purl=present,eval=present,echo=present,fig.height=7}
ggplot(finch@data, aes(map_id = id)) +
    expand_limits(x = pfinch$long, y = pfinch$lat)+
...
Print a summary table
```{r, results='asis', purl=present,eval=present,echo=present,fig.height=7}
xtable(summary(m1)$p.table)%>%
    print(type="html")
```

Compare all models
```{r,results='asis',purl=present,eval=present,echo=present}
kable(AIC(ndvi.only, 
          space.only, 
          space.and.ndvi,
          m1,m2))
